...
class TestProviderRegistry:
    def test_register_and_get(self):
        ...
        p = FakeProvider(Provider.OPENAI)
        registry.register(p)
        assert registry.get(Provider.OPENAI) is p
        assert registry.get(Provider.QWEN) is None

    def test_all(self):
        registry = ProviderRegistry()
        registry.register(FakeProvider(Provider.OPENAI))
    ...
    async def test_complete_with_fallback_success(self):
        registry = ProviderRegistry()
        openai = FakeProvider(Provider.OPENAI)
    ...
    async def test_complete_with_fallback_on_failure(self):
        registry = ProviderRegistry()
        bad_openai = FakeProvider(Provider.OPENAI, fail=True)
        qwen = FakeProvider(Provider.QWEN)
        registry.register(bad_openai)
        registry.register(qwen)
    ...
    async def test_complete_with_fallback_all_fail(self):
        registry = ProviderRegistry()
        registry.register(FakeProvider(Provider.OPENAI, fail=True))
